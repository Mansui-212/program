...
import { getUserSubmissions } from '@/api/modules/users'
import { useAuthStore } from '@/stores/auth'
import type { PublicSubmission } from '@/types/user'
...
async function loadSubmissions() {
  ...
  try {
    const response = await getUserSubmissions(formatUid(authStore.user.id))
    submissions.value = response.data
  } catch (requestError) {
    console.error('加载我的投稿失败', requestError)
    error.value = '投稿内容暂时无法加载，请稍后重试。'
  } finally {
    loading.value = false
  }
}
...
<template>
  <main class="submission-page">
    <section class="page-hero">
      <div>
        <p class="section-kicker">MY CONTRIBUTIONS</p>
        <h1>我的投稿</h1>
        <p>这里展示仍在基米小站公开收录的表情包与音乐。</p>
      </div>

      <RouterLink to="/profile" class="back-link">返回个人中心 →</RouterLink>
    </section>

    <p v-if="loading" class="page-status">正在整理你的投稿...</p>
    ...
    <template v-else>
      <section class="submission-section">
        ...
        <p v-if="memes.length === 0" class="empty-copy">还没有公开发布的表情包。</p>
      ...
      <section class="submission-section">
        ...
        <p v-if="music.length === 0" class="empty-copy">还没有公开发布的音乐。</p>
